...
function update_status_delete($color, $val) {
	...
	if(($status['w_setup'] == 1 && $status['b_setup'] == 1) && $total_pieces < 3) {
		$final_sql = "update game_status set status='aborded', result=?";
...
function update_game_status() {
	...
	$st3=$mysqli->prepare('select count(*) as aborded from players WHERE last_action< (NOW() - INTERVAL 5 MINUTE)');
	$st3->execute();
	$res3 = $st3->get_result();
	$aborded = $res3->fetch_assoc()['aborded'];
	if($aborded>0) {
